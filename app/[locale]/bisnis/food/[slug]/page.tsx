...
import { getFoodBrandBySlug, getFoodBrandSlugs } from "@/data/data-bisnis-food";
...
export function generateStaticParams() {
  return getFoodBrandSlugs().map((slug) => ({ slug }));
}
...
export async function generateMetadata({
  ...
  const brand = getFoodBrandBySlug(slug);
  if (!brand) return {};
  ...
  return {
    title: `${brand.name} | Food & Beverage Distribution`,
    description: desc,
    openGraph: {
      title: `Reddmasgroup | ${brand.name}`,
      description: desc,
      url: `${siteUrl}/bisnis/food/${slug}`,
    },
    alternates: {
      canonical: `${siteUrl}/bisnis/food/${slug}`,
      languages: {
        "id-ID": `${siteUrl}/id/bisnis/food/${slug}`,
        "en-US": `${siteUrl}/en/bisnis/food/${slug}`,
      },
    },
  };
}
