...
import { getHvacBrandBySlug, getHvacBrandSlugs } from "@/data/data-bisnis-hvac";
...
export function generateStaticParams() {
  return getHvacBrandSlugs().map((slug) => ({ slug }));
}
...
export async function generateMetadata({
  ...
  const brand = getHvacBrandBySlug(slug);
  if (!brand) return {};
  ...
  return {
    title: `${brand.name} | HVAC Installation & Services`,
    description: desc,
    openGraph: {
      title: `Reddmasgroup | ${brand.name}`,
      description: desc,
      url: `${siteUrl}/bisnis/hvac/${slug}`,
    },
    alternates: {
      canonical: `${siteUrl}/bisnis/hvac/${slug}`,
      languages: {
        "id-ID": `${siteUrl}/id/bisnis/hvac/${slug}`,
        "en-US": `${siteUrl}/en/bisnis/hvac/${slug}`,
      },
    },
  };
}
