...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: "Berita",
    description:
      "Seputar berita yang dilakukan seluruh jajaran karyawan Reddmasgroup",
    openGraph: {
      title: " Reddmasgroup | Berita",
      description:
        "Seputar berita yang dilakukan seluruh jajaran karyawan Reddmasgroup",
...
export default async function BeritaPage() {
  ...
  const items = [...beritaperusahaan].sort(
  ...
  const featuredText = featuredItem
    ? getBeritaText(beritaMessages, featuredItem.id)
    : null;
  return (
    <div className="w-full py-20">
      ...
      <section id="Berita">
        <main className="max-w-7xl mx-auto px-6 md:px-12 pt-48 md:pt-80 pb-20">
          {/* 1. FEATURED SECTION (berita Terbaru - Desain Sinematik) */}
                  ...
                  <div className="lg:col-span-5 flex flex-col justify-center">
                    <div className="flex items-center gap-3 mb-4">
                      <span className="text-xs font-semibold tracking-widest text-reddmas-red uppercase">
                        {t("berita.header_news")}
                      </span>
                      <span className="text-xs text-gray-400">•</span>
                      <span className="text-xs text-gray-500 font-medium">
                        {formatBeritaDate(featuredItem.date)}
                      </span>
                    </div>
          ...
          {/* 2. GRID SECTION (Sisa berita - Elegan & Bersih) */}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-x-12 gap-y-16 md:gap-y-24">
                {remainingItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <Link
                        href={`/ruang-informasi/berita/${item.slug}`}
